...
impl Command for ServerCommand {
    ...
    fn ident(&self) -> &str {
        ...
        match self {
            Handshake(_) => "HI",
            KeepAlive => "CHECK",
            Decryptor(_) => "decryptor",
            BanReason(_) => "BD",
            ServerVersion(_, _, _) => "ID",
            PlayerCount(_, _) => "PN",
        }
    }
    ...
    fn extract_args(&self) -> Vec<String> {
        ...
        match self {
            Handshake(str) => vec![str.clone()],
            KeepAlive => vec![],
            Decryptor(i) => vec![i.to_string()],
            BanReason(reason) => vec![reason.clone()],
            ServerVersion(client_id, software, version) => {
                vec![client_id.to_string(), software.clone(), version.clone()]
            }
            PlayerCount(p_c, p_m) => vec![p_c.to_string(), p_m.to_string()],
        }
    }
}
